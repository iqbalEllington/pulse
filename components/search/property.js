...
const SearchProperty = (props) => {
    ...
    async function getProperties(value) {
        var response;
        if (value != false) {
            response = await getRequest({ API: API_URLS.GET_PROPERTIES + '?pagination[page]=1&filters[occuarance]=All Time&pagination[pageSize]=100&populate[]=featuredImage&filters[name][$containsi]=' + value + '&sort=launchDate:desc' });
        } else {
            response = await getRequest({ API: API_URLS.GET_PROPERTIES + '?pagination[page]=1&filters[occuarance]=All Time&pagination[pageSize]=100&populate[]=featuredImage&sort=launchDate:desc' });
        }
        var data = []
